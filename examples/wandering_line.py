...
darm = lib.butter_filter(darm, low=low, high=high, fs=fs)

data = TimeSeries.get(chan1, st, st + dur, nproc=4)
if data.sample_rate.value != fs:
    data = data.resample(fs)
wit = data.value
wit = lib.butter_filter(wit, low=low, high=high, fs=fs)
...
specgram = ts.spectrogram(2, fftlength=1, overlap=0.5) ** (1 / 2.0)
...
specgram = dts.spectrogram(2, fftlength=1, overlap=0.5) ** (1 / 2.0)
plot = specgram.imshow(norm="log", vmin=mn / 10, vmax=mx * 10)
ax = plot.gca()
ax.set_yscale("log")
ax.set_ylim(10, 200)
